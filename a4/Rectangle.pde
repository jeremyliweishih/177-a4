...
class Rectangle {
  ...
  float getWidth() {
    return (this.parent == null ? chartW : this.parent.getWidth()) * this.w;
  }
  
  float getHeight() {
    return (this.parent == null ? chartH : this.parent.getHeight()) * this.h;
  }
  
  float getX() {
   
    
    return this.parent == null ? chartW * this.x: parent.getX() + parent.getWidth() * this.x;
  }
  
  float getY() {
    return this.parent == null ? chartH * this.y: parent.getY() + parent.getHeight() * this.y;
  }
  ...
  private float getROff() {
    ...
    float absX = getX();
    float absW = getWidth();
    return (Math.abs(absX + absW - this.chartW) < EPSILON ||
  ...
  private float getDOff() {
    ...
    float absY = getY();
    float absH = getHeight();
    return (Math.abs(absY + absH - this.chartH) < EPSILON ||
